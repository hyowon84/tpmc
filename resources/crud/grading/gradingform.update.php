...
if($mode == 'statsUpdate') {
	...
	for($i = 0; $i < count($grid); $i++) {
		$order = $grid[$i];
		$number_list .= "'$order[no]',";

		$log_sql = "INSERT INTO 		log_table	SET
																	logtype = 'grading_item',		/*로그유형 ( ex: clayorder )*/
																	gr_id = '$order[gr_id]',							/*pk_id*/
																	pk_id = '$order[no]',
																	memo = '그레이딩목록 상태변경',			/*메모태그*/
	...
	$common_sql = "	UPDATE	grading_item	SET
														gr_stats = '$stats'
									WHERE		no IN ($number_list)
	";
	$result = $sqli->query($common_sql);
	db_log($common_sql,'grading_item','그레이딩신청목록 상태 변경');

	if($result) {
		$json[success] = "true";
		$json[message] = '그레이딩신청목록의 상태가 변경되었습니다';
	} else {
		$json[success] = "false";
		$json[message] = '그레이딩신청목록의 상태 변경이 실패하였습니다. 관리자에게 문의바랍니다.';
	}
...
//		$환불변경값 = " ,refund_money = '$refund_money' ";
	...
	//송장번호 일괄변경
	$common_sql1 = "	UPDATE	grading_item	SET
														shipping_no = '$delivery_invoice'
														$환불변경값
									WHERE		no IN ($number_list)
	";
	$result = $sqli->query($common_sql1);
	db_log($common_sql1, 'graidng_item', '통합배송관리 주문정보별 배송정보 입력');
...
if($result) {
	$json[success] = "true";
	$json[message] = '주문내역들이 변경되었습니다';
} else {
	$json[success] = "false";
	$json[message] = '주문내역들의 수정이 실패하였습니다. 관리자에게 문의바랍니다.';
}
